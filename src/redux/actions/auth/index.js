...
import { authBaseUrl, baseAxios, merchandisingAxiosInstance } from '../../../services';
...
import { AUTH_TOKEN_STORE, BIND_DEFAULT_TENANT, BIND_USER_TENANT, GET_AUTHENTICATE_USER, GET_AUTHENTICATE_USER_PERMISSION, IS_OPEN_INSTANT_LOGIN_MODAL, IS_USER_LOGGED_IN, LOGOUT, SESSION_DURATION } from '../../action-types';
...
export const bindUserTenant = ( defaultTenant ) => dispatch => {
  const { defaultTenantId } = defaultTenant;
  authBaseUrl.defaults.headers.common['TenantId'] = `${defaultTenantId}`;
  merchandisingAxiosInstance.defaults.headers.common['TenantId'] = `${defaultTenantId}`;
  baseAxios.defaults.headers.common['TenantId'] = `${defaultTenantId}`;

  dispatch( {
    type: BIND_USER_TENANT,
    defaultTenant,
    defaultTenantId
  } );
};

export const bindDefaultTenant = ( defaultTenantId ) => dispatch => {
  authBaseUrl.defaults.headers.common['TenantId'] = `${defaultTenantId}`;
  merchandisingAxiosInstance.defaults.headers.common['TenantId'] = `${defaultTenantId}`;
  baseAxios.defaults.headers.common['TenantId'] = `${defaultTenantId}`;

  dispatch( {
    type: BIND_DEFAULT_TENANT,
    defaultTenantId
  } );
};
...
export const getAuthUser = ( push ) => async dispatch => {
  ...
  await authBaseUrl.get( apiEndPoint ).then( response => {
    if ( response.status === status.success ) {
      ...
      dispatch( bindUserTenant( defaultTenant ) );
