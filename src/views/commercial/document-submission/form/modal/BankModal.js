...
import { bindDocumentSubmissionInfo } from "../../store/actions";
...
const BankModal = ( props ) => {
    ...
    const handleRow = ( row ) => {
        ...
        dispatch( bindDocumentSubmissionInfo( updatedInfo ) );
        handleModalClose();
    };
            ...
            <UILoader
                ...
                <DataTable
                    conditionalRowStyles={[
                        {
                            when: row => row.value === documentSubInfo[whichForTheModal]?.value.toLowerCase().trim() ?? '',
                            style: {
