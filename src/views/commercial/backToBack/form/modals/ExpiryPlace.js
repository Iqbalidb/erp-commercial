...
const ExpiryPlace = ( props ) => {
    ...
    const rowSelectCriteria = ( row ) => {
        const filteredData = !single ? backToBackInfo[whichForTheModal]?.find( d => d.label === row.label ) : null;
        return filteredData;
    };
    ...
    const handleRowDoubleClick = ( data ) => {
        ...
        if ( single ) {
            if ( backToBackInfo ) {
                ...
                dispatch( bindBackToBackInfo( updatedMasterDocument ) );
            }
            handleModalClose();
            //  setOpenModal( false );
            ...
            >
                ...
                <DataTable
                    conditionalRowStyles={[
                        {
                            when: row => ( single ? row.value === backToBackInfo[whichForTheModal]?.value ?? '' : backToBackInfo[whichForTheModal].find( p => p.label === row.label )?.label === row.label ?? '' ),
                            style: {
