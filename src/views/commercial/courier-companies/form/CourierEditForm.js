...
import { bindCourierCompanyInfo, editCourierCompany } from "../store/actions";

const CourierEditForm = ( props ) => {
    ...
    const { contactInfo } = couerierInfo;
    console.log( { couerierInfo } );
    const [contactInfos, setContactInfos] = useState( contactInfo );
    const emptyContactNumberCheck = contactInfos.some( c => !c.contactNumber.length > 0 );

    const isEveryContactNumberValidated = () => {
        const isValidated = contactInfos.every( c => c.contactNumber && c.contactPerson );
        return isValidated;
    };
    ...
    const handleContactInfoDelete = ( row ) => {
        confirmDialog( confirmObj )
            .then( e => {
                if ( e.isConfirmed ) {
                    const updatedRow = contactInfos.filter( r => r.id !== row.id );
                    setContactInfos( updatedRow );
                }
            } );
    ...
    const handleContactInfoChange = ( e, id ) => {
        const { name, value } = e.target;
        const updatedRows = contactInfos.map( ( cn ) => {
            if ( cn.id === id ) {
                // return {
                //     ...cn,
                //     cn[name] = value;
    ...
    const onSubmit = () => {
        const submitObj = {
            ...
            contactInfo: JSON.stringify( contactInfos?.map( c => ( { contactNumber: c.contactNumber, contactPerson: c.contactPerson } ) ) ),
            status: true
        };
        dispatch(
            editCourierCompany(
                ...
                <div >
                    ...
                    <div className='mt-1'>
                        ...
                        <table className='table contact-number-table table-bordered' >
                            ...
                            <tbody>
                                {
                                    contactInfos && contactInfos.map( ( el, i ) => {
                                        return (
                                            <tr key={i}>
