...
import { getBackToBackDropdownCm, getBuyerDropdownCm, getExportInvoiceDropdownCm, getFOCDropdownCm, getGeneralImportDropdownCm, getMasterDocumentDropdownCm, getSupplierDropdown } from "redux/actions/common";
...
import { bindGeneralChargeAdviceDetails, bindGeneralChargeAdviceInfo, getGeneralChargeAdviceByQuery } from "../store/actions";
import GenralChargeAviceColumns from "./GenralChargeAviceColumns";

const GeneralChargeAdviceList = () => {
    ...
    const { allData, total } = useSelector( ( { generalChargeAdviceReducer } ) => generalChargeAdviceReducer );
    ...
    const handleGetAllGeneralChargeAdvice = () => {
        dispatch( getGeneralChargeAdviceByQuery( paramsObj, filteredData ) );
    };
    useEffect( () => {
        handleGetAllGeneralChargeAdvice();
    }, [dispatch] );
    ...
    const handleSearch = () => {
        dispatch(
            getGeneralChargeAdviceByQuery( { ...paramsObj, page: 1 }, filteredData )
        );
    };
    ...
    const handleClearFilterBox = () => {
        ...
        dispatch( getGeneralChargeAdviceByQuery( { ...paramsObj, status: true }, [] ) );
    };
    ...
    const handleRefresh = () => {
        ...
        setFilteredArray( defaultFilteredArrayValue );
        // setStatus( true );
        dispatch( getGeneralChargeAdviceByQuery( {
            ...paramsObj,
            page: 1,
            perPage: 10,
            status: true,
    ...
    const handleAddNew = () => {
        push( '/new-general-charge-advice' );
    ...
    return (
        <>
            <ActionMenu breadcrumb={breadcrumb} title='General Charge Advices' >
            ...
            <UILoader
                ...
                <FormLayout isNeedTopMargin={true} >
                    ...
                    <DataTable
                        ...
                        highlightOnHover
                        data={allData}
                        columns={GenralChargeAviceColumns( handleDetails )}
                        dense
                        // expandableRows={true}
...
export default GeneralChargeAdviceList;