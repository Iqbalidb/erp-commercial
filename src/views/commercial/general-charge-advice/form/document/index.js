...
import { generalChargeAdviceFileDelete, generalChargeAdviceFileUpload } from "../../store/actions";
...
const GeneralChargeAdviceDocument = ( { isDetailsForm = false } ) => {
    const dispatch = useDispatch();

    const {
        isFileUploadComplete
    } = useSelector( ( { commonReducers } ) => commonReducers );
    const { generalChargeAdviceInfo
    } = useSelector( ( { generalChargeAdviceReducer } ) => generalChargeAdviceReducer );
    const { files, fileUrls } = generalChargeAdviceInfo;
    ...
    const handleAddFileToTable = () => {
        ...
        dispatch( generalChargeAdviceFileUpload( obj ) );
    ...
    const handleFileRemoveFromTable = file => {
        confirmDialog( confirmObj ).then( async e => {
            if ( e.isConfirmed ) {
                ...
                dispatch( generalChargeAdviceFileDelete( file, handleDeleteProgress ) );
            }
        } );
    };
...
export default GeneralChargeAdviceDocument;