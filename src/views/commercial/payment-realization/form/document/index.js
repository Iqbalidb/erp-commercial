...
import { paymentRealizationFileDelete, paymentRealizationFileUpload } from "../../store/actions";

const PaymentRealizationDocument = ( { isDetailsForm = false } ) => {
    const initialFilesUpload = {
        ...
        file: null,
        date: '',
        for: 'PaymentRealization',
        documentCategory: null
    };
    const dispatch = useDispatch();
    const { isFileUploadComplete } = useSelector( ( { commonReducers } ) => commonReducers );
    const { paymentRealizationInfo } = useSelector( ( { paymentRealizationReducer } ) => paymentRealizationReducer );
    const { files, fileUrls } = paymentRealizationInfo;
    ...
    const handleAddFileToTable = () => {
        ...
        dispatch( paymentRealizationFileUpload( obj ) );
    ...
    const handleFileRemoveFromTable = file => {
        confirmDialog( confirmObj ).then( async e => {
            if ( e.isConfirmed ) {
                ...
                dispatch( paymentRealizationFileDelete( file, handleDeleteProgress ) );
            }
        } );
    };
...
export default PaymentRealizationDocument;