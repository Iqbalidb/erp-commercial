...
import { bindGeneralImportInfo } from "../../store/actions";

const ExpiryPlaceModal = ( props ) => {
    ...
    const rowSelectCriteria = ( row ) => {
        const filteredData = !single ? generalImportInfo[whichForTheModal]?.find( d => d.label === row.label ) : null;
        return filteredData;
    };
    ...
    const handleRowDoubleClick = ( data ) => {
        ...
        if ( single ) {
            ...
            handleModalClose();
            //  setOpenModal( false );
            // dispatch( getCountryPlaceDropdownCm( '' ) );
        }
    };
    return (
        <CustomModal
            ...
            >
                ...
                <DataTable
                    conditionalRowStyles={[
                        {
                            when: row => ( single ? row.value === generalImportInfo[whichForTheModal]?.value ?? '' : generalImportInfo[whichForTheModal].find( p => p.label === row.label )?.label === row.label ?? '' ),
                            style: {
...
export default ExpiryPlaceModal;