...
const PortsModalCI = ( props ) => {
    ...
    const rowSelectCriteria = ( row ) => {
        const filteredData = !single ? commercialInvoiceInfo[whichForTheModal]?.find( d => d.label === row.label ) : null;
        return filteredData;
    };
    ...
    const handleRowDoubleClick = ( data ) => {
        ...
        if ( single ) {
            if ( commercialInvoiceInfo ) {
                ...
                dispatch( bindCommercialInvoiceInfo( updateExportInvInfo ) );
            ...
            >
                ...
                <DataTable
                    conditionalRowStyles={[
                        {
                            when: row => ( single ? row.value === commercialInvoiceInfo[whichForTheModal]?.value ?? '' : commercialInvoiceInfo[whichForTheModal]?.find( p => p.label === row.label )?.label === row.label ?? '' ),
                            style: {
