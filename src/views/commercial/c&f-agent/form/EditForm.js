...
import { bindAllAgentInfo, updateAgent } from '../store/actions';
...
const EditForm = ( props ) => {
    ...
    const editAgentSchema = yup.object().shape( {
        ...
        country: agentInfo.country ? yup.string() : yup.string().required( 'Type is Required!!!' ),
        ...
        postalCode: agentInfo.postalCode.trim().length ? yup.string() : yup.string().required( 'Account Number is Required!!!' ),
        agentType: agentInfo.agentType?.length ? yup.string() : yup.string().required( 'Type is required' )
    } );
    const { errors, reset, handleSubmit } = useForm( { mode: 'onChange', resolver: yupResolver( editAgentSchema ) } );
    ...
    const onSubmit = () => {
        ...
        dispatch( updateAgent( submitObj ) );
