...
import { bindEDFInfo } from "../../store/actions";
import BackToBackModalEDFColumns from "./BackToBackModalEDFColumns";

const BackToBackModalEDF = ( props ) => {
    ...
    const handleRow = ( row ) => {
        if ( row.id === edfInfo?.backToBackNo?.value || edfInfo?.backToBackNo === null ) {
            const updatedBackToBack = {
                ...
                bankBranch: { label: row.openingBankBranch, value: row.openingBranchId },
                bbAmount: row.documentAmount,
                backToBackAmount: row.documentAmount
            };
        ...
        >
            ...
            <DataTable
                conditionalRowStyles={[
                    {
                        when: row => row.documentNumber === edfInfo?.backToBackNo?.label,
                        style: {
                ...
                expandOnRowClicked
                columns={BackToBackModalEDFColumns()}
...
export default BackToBackModalEDF;