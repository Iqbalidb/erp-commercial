...
import { Maximize2, Minimize2, Trash2 } from 'react-feather';
import { useDispatch, useSelector } from 'react-redux';
import SlideDown from 'react-slidedown';
import { Button, Label, Table } from 'reactstrap';
import { isZeroToFixed } from '../../../../../utility/Utils';
import { confirmDialog } from '../../../../../utility/custom/ConfirmDialog';
import { bindRemovableSizeColorQty, bindTransFerableList } from '../../store/actions';
const ExpandableMasterDocPurchaseOrderQuantitySizeAndColorForm = ( { data, fromTransferDetails } ) => {
        ...
        confirmDialog( { ...confirmObj, text: `The <span class="text-danger font-bolder" >${size.size}</span>  will be removed!!` } )
            .then( e => {
                ...
                if ( e.isConfirmed ) {
                    ...
                    const qtyRemoveRows = data?.orderQuantitySizeAndColor.filter( qty =>
                    ( qty.orderId === size.orderId &&
                        qty.colorId === size.colorId &&
                        qty.sizeId === size.sizeId )
                    );
                    const removableQty = qtyRemoveRows.map( qty => ( {
                        id: qty.id,
                        beneficiaryId: qty.beneficiaryId
                    } ) );
                                                    ...
                                                    <Table size="sm" bordered hover responsive >
                                                        <thead className="thead-light" >
                                                            <tr className="text-center" >
                                                                {!fromTransferDetails && <th className='text-nowrap'>Action</th>}
                                                                {/* <th className='text-nowrap'>BE</th> */}
                                                        ...
                                                        <tbody>
                                                            {
                                                                sortedSizeColorQuantity.filter( sc => sc.color === color ).map( ( size, sIndex ) => {
                                                                    return (
                                                                        <tr key={sIndex}>
                                                                            {!fromTransferDetails && <td style={{ width: '10px', textAlign: 'center' }}>
                                                                                <span style={{ cursor: 'pointer' }}
                                                                                    onClick={() => handleRemoveSize( size, data.beneficiaryId )}>
...
export default ExpandableMasterDocPurchaseOrderQuantitySizeAndColorForm;