...
import { bindExportInvoiceInfo, deleteExportInvoice, getAllExportInvoicesByQuery } from "../store/actions";
...
const ExportInvoiceList = () => {
    ...
    const { allData, total } = useSelector( ( { exportInvoiceReducer } ) => exportInvoiceReducer );
    ...
    const handleGetAllExportInvoice = () => {
        dispatch( getAllExportInvoicesByQuery( paramsObj, filteredData ) );
    };

    useEffect( () => {
        handleGetAllExportInvoice();
    }, [dispatch, isDraft] );
    ...
    const handleRefresh = () => {
        ...
        // setStatus( true );
        dispatch( getAllExportInvoicesByQuery( {
            ...paramsObj,
            page: 1,
            perPage: 10,
            status: true
    ...
    const handleSearch = () => {
        dispatch(
            getAllExportInvoicesByQuery( { ...paramsObj, page: 1 }, filteredData )
        );
    };
    ...
    const handleClear = () => {
        setFilterObj( defaultFilterValue );
        setFilteredArray( defaultFilteredArrayValue );
        dispatch( getAllExportInvoicesByQuery( { ...paramsObj, status: true }, [] ) );
        setCurrentPage( 1 );
        setRowsPerPage( 10 );
    };
    ...
    const handleDelete = ( row ) => {
        confirmDialog( confirmObj )
            .then( e => {
                if ( e.isConfirmed ) {
                    dispatch( deleteExportInvoice( row.id ) );
                    setCurrentPage( 1 );
                }
            }
            );
    ...
    const handleDetails = ( row ) => {
        push( {
            pathname: '/export-invoice-details',
            state: row.id
        } );
    };
    const handleTab = ( tab ) => {
    ...
    const handleAddNew = () => {
        push( '/new-export-invoice' );
        dispatch( bindExportInvoiceInfo( null ) );
    };
    return (
        <>
            <ActionMenu breadcrumb={breadcrumb} title='Export Invoices' >
                ...
                <TabContainer tabs={tabs} onClick={handleTab}>
                    <div>
                        <DataTable
                            ...
                            columns={ExportInvoiceColumn( handleEdit, handleDelete, handleAmendment, handleDetails )}
